...
aws emr add-steps --cluster-id $clusterID --steps Name="Command Runner",Jar="command-runner.jar",Args=["s3-dist-cp","--s3Endpoint=s3.amazonaws.com","--src=s3://xds-data/AG/SDA-DATA/fma2_v13_featuredata.edd_iPerl.parquet","--dest=hdfs:///user/hadoop/AG/fma2_v13_featuredata.edd_iPerl.parquet/"]
